...
abcInt obj;
int a = obj(-1);	// 调用 absInt.operator() 函数
```

> 如果类定义了函数调用运算符, 则类的对象称为函数对象, 因为可以像函数一样调用类的对象
...
Print print;
print(s);			// 在 cout 中打印 s, 再输出一个空格

Print error(cerr, '\n');
error(s);			// 在 cerr 中打印 s, 再出书一个换行符
```

**函数对象作为泛型算法的实参**

```cpp
for_each(v.begin(), v.end(), Print(cerr, '\n'));
// 第三个实参是 Print 类的一个临时对象, 用 cerr 和 '\n' 来初始化
```

#### 14.8.1 lambda 是函数对象

编译器会把 lambda 表达式翻译为一个**未命名类的未命名对象**, 并重载了函数调用运算符
...
public:
    ...
    { return a.size() < b.size(); }		// 由于默认 lambda 不能修改捕获的变量, 所以默认是 const 函数
};
```
...
find_if(v.begin(), v.end(), Class(sz));
```

#### 14.8.2 标准库定义的函数对象

标准库定义了一组表示**算术**, **关系**和**逻辑**运算符的类, 并定义了相应操作的调用运算符
...
```cpp
plus<int> add;		// 可执行 int 加法的函数对象
int res = add(2, 3);

negate<int> neg;	// 对 int 取反的函数对象
int res = neg(-5);	
```
...
```cpp
sort(v.begin(), v.end(), greater<string>());	// 降序排列
```

**标准库函数对象也适用于指针**

```cpp
[](string *a, string *b) { return a < b; } 		// 错误, 不能直接比较指针地址大小
sort(v.begin(), v.end(), less<string*>());		// 正确, 标准库函数对象 less<string> 可以比较指针地址大小
```
...
| function 的操作             | 说明                                                         |
| --------------------------- | ------------------------------------------------------------ |
| function<T\> f;             | f 是一个 function<T\> 的对象, 可用来存储可调用对象<br />调用形式与 T 相同, T 为 retType(args) |
| function<T\> f(nullptr);    | 显式构造一个空 function<T\> 对象                             |
| function<T\> f(obj);        | 用可调用对象 obj 初始化 f                                    |
| f                           | f 作为条件: f 含有可调用对象时为真, 否则为假                 |
| f(args)                     | 调用 f 中的可调用对象, 参数为 args                           |
| **function<T\> 成员的类型** |                                                              |
...
```cpp
function<int(int, int)>		// 调用形式为 int(int, int) 的可调用对象类
    
function<int(int, int)> f1 = add;			// 函数
function<int(int, int)> f2 = [](int a, int b) { return a % b; };	// lambda 表达式
function<int(int, int)> f3 = divide();		// 重载了 () 的类对象
...
```cpp
// 函数指针
int (*fp)(int, int) = add;		// 接受两个 int 的版本
m.insert({"+", fp});

// lambda
m.insert({"+", [](int a, int b) { return add(a, b); }});
```
...
```cpp
operator type() const;		// 成员函数, 无返回类型, 将类类型转化为 type 类型
...
Class c;
c = 4;		// 构造函数定义的隐式类型转换: 将 4 转化为 Class 类型, 然后调用 Class::operator=
c + 3;		// operator int() const 定义的类型转换: 将 c 转化为 int 类型

Class c = 3.14;		// 内置类型转换: double -> int, 然后调用 Class(int) 构造函数 
c + 3.14;			// 先 c 转换为 int, 然后内置类型转换: int -> double
```
...
Class c = 3.14;		// 调用 Class(int) 构造函数
static_cast<int>(c) + 3;	// 显式类型转换

// 例外: 当表达式用作条件时, 显式类型转换会自动被使用
```
...
{
    A() = default;
    A(const B &);	// 隐式类型转换: B -> A
};

struct B
{
    operator A() const;	// 隐式类型转换: B -> A
};

// 都定义了 B -> A 的类型转换, 产生二义性错误
```
...
struct A
{
    A(int);						// int -> A			
    A(double);					// double -> A
    operator int() const;		// A -> int
    operator double() const;	// A -> double
};
double -> A 路线:
	double -> int -> A
    double -> A
...
fun(5);			// 二义性错误, int -> A, int -> double -> B
```
...
表达式中运算符的候选函数既包括**成员函数**, 也应包括**非成员函数**

```cpp
class Class
{
    friend
    Class operator+(const Class&, const Class&);
public:
    Class(int);		// int -> Class
    operator int() const { return val; }	// Class -> int
private:
    size_t val;
};
Class c1, c2;
Class res = c1 + c2;	// 调用重载的 operator+
int a = res + 1;		// 二义性错误
// 1 -> Class, 然后执行 Class 的加法操作
// res -> int, 然后执行 int 的加法操作
```
