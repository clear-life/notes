...
**预处理**时进行**简单替换**, 不进行**参数检测**和**成员访问控制**检测
...
> **底层 const** 看作一种**新的类型**, 顶层 const 不看做一种新的类型
>
> 权限: 底层 const 权限高, 顶层 const 权限低

**顶层 const** 可以通过指针/引用**修改**指向内存的值

**底层 const** **不可以**通过指针/引用修改指向内存的值

**const 成员函数** **不可以**修改对象的值, **const 成员函数**也是**不同的类型**
...
> 底层 const 相当于权限高, 可以**同时接收**权限高和低的类型
>
> 顶层 const 相当于权限低, 不能接收权限比自己高的底层 const 

### volatile

volatile 表明该变量随时有可能被修改, 告诉编译器不要对其优化到寄存器, 要一直**通过内存地址取值**
...
多态机制中**父子类指针/引用**的类型转换

down: **父转子**, 借助 RTTI 检测是否安全, 失败返回 0

不安全情况:**基类指针指向基类对象**, 把该**基类指针转换为子类指针**会不安全, 因为转换后的子类指针实际并没有子类对象独有的内容
...
当进行**写操作**后, **引用计数减一**, **开辟新的空间**, **新空间**的引用计数**加一**

### C 和 C++ 区别

C++ 比 C 多了**面向对象**, **STL**, **模板**, **泛型编程**, **元编程**
...
在类中**重载函数调用运算符** `()`, 可以**把类的对象当作函数**来使用, 称为**函数对象**或**仿函数**
...
对 lambda 表达式, 编译器自动生成一个匿名类(重载 `()` 运算符), 闭包类型, 返回**匿名的闭包实例**, 右值

**匿名类**

捕获的变量 = 类的非 static 数据成员

lambda 表达式本身 = 类的成员函数(默认是 const, 不可以修改捕获的变量)

> mutable 关键字: 调用普通成员函数, 可以修改捕获的变量
>
> 引用成员: 无论是否加 mutable 关键字, 都可以修改捕获的引用成员
...
> **父类指针限制**调用函数, 然后通过**子类对象的 vptr 访问子类对象的虚表**, 再进而访问虚函数

父类指针做**框架限制**, 子类对象做**实际调用**, 实际调用要在**框架限制之内**
...
**不可以**, 虚函数的目的是多态, 但**子类并不继承父类的构造函数, 不可能被重写**
...
立意: static 成员不属于类对象, 即使加上 virtual 也没有意义

机制: **static 成员函数没有 this 指针**, **无法访问 vptr**, 不支持多态机制
...
**基类指针指向子类对象**时, delete 基类指针, **调用子类析构函数**, 子类析构函数又会调用基类析构函数, 这样就能**正常释放**子类对象

> 基类指针指向子类对象时, 析构顺序需要从下往上, 所以**基类指针需要调用子类析构函数**
...
这样, 在 delete 基类指针时, 能够实现由**子类到基类**的**析构函数链式调用**
...
**子类对象**可以通过直接访问前四个字节(即 vptr) 访问**虚表**, 进而访问到**父类对象的 private 虚函数**
...
作用: **不实例化对象**, 只有**重新定义父类的所有纯虚函数**, 才能实例化对象
...
**可以**是纯虚函数, 但**必须定义函数体**, 因为**子类不能实现基类析构函数(因为不继承析构函数)**, 而且基类析构函数会在子类析构函数中调用
...
隐含 **this 指针**, 可以调用**静态成员**和**非静态成员**

### 成员函数的调用

**编译**时确定调用哪个非 virtual 函数

**运行**时确定调用哪个 virtual 函数
...
C++ **全局对象**和 **static 对象**首次用到时才**构造并初始化**, 按照**构造的反方向析构**
...
作用: const 修饰的成员函数**不能修改数据成员**

**本质**: **把函数看作指针**, 访问数据成员看作指针访问指向的对象, **不能修改数据成员**看作**不能修改指针指向的对象**, 把**对象看作类型**

那么 **const 成员函数**就**相当于**是**底层 const** 了

> 或者说**函数本身就是顶层 const 指针**, **const 函数**相当于是**底层 const 指针**, 同时也是顶层 const 指针

于是, const 修饰成员函数可以用来区分**函数重载**
...
> 因为后一个构造的可能会依赖于前一个构造的对象, 所以要**拓扑构造**
>
> 析构时也一样, 要**反向拓扑析构**
>
> 拓扑图的反向也是一个拓扑图
