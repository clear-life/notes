...
4. **链接**
   ...
   > 将文件 A 使用的符号与文件 B 的定义链接起来
   >
   > **链接 = 符号解析 + 重定位**
   >
   > 符号解析:
   >
   > 每个文件生成符号表: 导出符号表 + 未解决符号表
   >
   > 重定位:
   >
   > 每个编译单元从零地址开始，多个编译单元链接时需要对每个编译单元的地址进行调整，这就是地址重定位
   >
   > https://zhuanlan.zhihu.com/p/346787730

​		静态链接: 编译阶段将**库文件代码加到可执行文件**

​			优点: 加载速度快，效率高，**不依赖外部库环境**

​		动态链接: **运行时加载动态库**

​			优点: 可执行文件小，库文件修改时**不需要重新编译**
...
| 方向     | 由高到低       | 由低到高                       |
| 效率     | 高             | 低                             |
| 大小     | 固定(一般2M)   | 虚拟内存(一般4G)               |
| 碎片     | 不产生内存碎片 | 频繁 new/delete 会产生内存碎片 |
| 缓存     | 栈在一级缓存里 | 堆在二级缓存里                 |

> 操作系统有一个记录空闲内存地址的链表
>
> 遍历空闲链表, 找到第一个大于申请空间的堆节点, 然后将该节点分配给程序, 多余部分重新放入空闲链表
...
1. static 成员属于整个类
2. 声明时加 static, 初始化时不加 static, 在**类外初始化**

**构造与析构**

**拓扑构造, 反拓扑析构**

C 中 static 变量初始化发生在编译阶段

C++ **全局对象**和 **static 对象**首次调用时**构造并初始化**, 并按照**构造的反方向析构**
...
失败: 指针失败返回 NULL, 引用失败抛出 `std::bad_cast` 异常
...
**浅拷贝**, 拷贝前后的**指针指向同一内存**, 没有开辟新内存

**深拷贝**, 拷贝前后的**指针指向不同内存**, 开辟新内存
...
|        | 重载           | 重写           | 隐藏                       |
| ------ | -------------- | -------------- | -------------------------- |
| 作用域 | **相同**作用域 | **父子**作用域 | **不同**作用域             |
| 函数头 | 参数列表不同   | **函数头相同** | 参数列表可以相同, 可以不同 |
...
调用构造函数前, 虚表和 vptr 还未初始化, 不能执行多态机制

### 析构函数可以是虚函数吗

**可以**

虚表和 vptr 已初始化, 可以执行多态机制
...
纯虚函数也是虚函数, 虚表中存储其函数地址

|      | 虚函数     | 纯虚函数               |
| ---- | ---------- | ---------------------- |
| 目的 | 多态       | 抽象类                 |
| 定义 | virtual{}  | virtual{} = 0          |
| 实现 | 可以被重写 | 必须被定义才能生成对象 |
...
移动语义: 源对象的**资源控制权转交**给目标对象

移动构造: **临时对象**的**资源控制权转**交给**新构造的对象**
...
/*
1. fun 函数内构造 A 的对象 a
2. fun 函数返回临时对象(拷贝构造), a 销毁
3. fun().get() 调用临时对象的 get() 方法
4. cout 语句结束后, 临时对象销毁
*/
```
...
编译器在**栈**上分配内存, 直接调用**构造函数**创建对象

> A a;

**动态建立:** 

用 `new` 在**堆**上分配内存, 先调用 **`operator new()`** 分配内存, 再调用**构造函数**创建对象
...
**成员变量和类**的**初始化顺序**都按照**声明顺序**来初始化, **与初始化列表的顺序无关**
...
目的: 让**另一函数/类**, 访问当前类的**私有和保护成员**

> 类的友元能访问该类的所有成员(指权限, 具体还要看能不能访问的到)
