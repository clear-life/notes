...
**构造函数会屏蔽多态机制**, **把虚函数当作普通函数使用**

因为创建子类对象时, 先调用父类构造函数, **父类构造函数只会调用父类的虚函数**, 不会进行动态绑定
...
**构造函数不能被继承**, 不可能被子类重写, 没有必要设为虚函数

### 析构函数可以是虚函数吗

析构函数**必须是虚函数**

当**父类指针 new 子类对象**时, 需要动态绑定机制来**调用子类的析构函数**
