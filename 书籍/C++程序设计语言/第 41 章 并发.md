...
```C++
x.is_lock_free()			// x 是否用 lock-free 实现
x.store(t)					// x.val = t
x.store(t, memory_order)		
t = x.load()				// t = x.val
t = x.load(memory_order)
t2 = x.exchange(t)			// x->t2 t->x  
t2 = x.exchange(t, memory_order)
```
...
b = x.compare_exchange_weak(rt,t,o1,o2)	
    if x.val == rt
        ...
        memory_order = o2
        rt = x.val
b = x.compare_exchange_weak(rt,t,memory_order)
    
b = x.compare_exchange_strong(rt,t,o1,o2)
b = x.compare_exchange_strong(rt,t,memory_order)
...
考虑一个无锁单链表, 若 data 的值小于头节点的 data 值, 则头插法插入新节点

```C++
atomic<node*> head;

node* nh = new node(data, nullptr);
node* h = head.load();
...
nh 是要写入的新值, **h 代表 head 现在被当前线程看到的值**
